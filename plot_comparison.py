...
for i in range(2):
    for j in range(3):
        # axs_1[j, i].plot(t_int, absolute_error_int[:, i * 3 + j], label = '绝对误差(i)', color = 'blue')
        axs_1[j, i].plot(t_int, absolute_error_lagrange[:, i * 3 + j], label = '绝对误差(l)', color = 'orange')
...
for i in range(3):
    for j in range(2):
        ...
        axs_2[i, j].plot(t_lagrange, A_lagrange[:, i * 2 + j], label='拉格朗日系数法', color='orange')
